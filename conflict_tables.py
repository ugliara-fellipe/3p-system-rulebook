...
def generate_dice_table(selected_dice, max_aspect):
    ...
    for a in range(1,max_aspect+1):
        for b in range(1,max_aspect+1):
            ...
            value_a = '&larr; ' + '{0: >2}'.format(str(init_a)) + '&bull;' + '{0: <2}'.format(str(exit_a)) if init_a != exit_a else '&larr; ' + '{0: <5}'.format('{0: >3}'.format(init_a))
            value_b = '&uarr; ' + '{0: >2}'.format(str(init_b)) + '&bull;' + '{0: <2}'.format(str(exit_b)) if init_b != exit_b else '&uarr; ' + '{0: <5}'.format('{0: >3}'.format(init_b))
            mdf.iloc[a - 1, b - 1] = value_a + '<br/>'  + value_b

    return mdf

#rint(generate_dice_table(6, 5).to_markdown())
#print('')
